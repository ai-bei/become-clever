...
public final class OrderByValue implements Comparable<OrderByValue> {
    ...
    private List<Boolean> getOrderValuesCaseSensitive(final SelectStatementContext selectStatementContext, final SchemaMetaData schemaMetaData) {
        ...
        for (OrderByItem eachOrderByItem : orderByItems) {
            boolean orderValuesCaseSensitiveFromTables;
            try {
                orderValuesCaseSensitiveFromTables = getOrderValuesCaseSensitiveFromTables(selectStatementContext, schemaMetaData, eachOrderByItem);
            }catch (NullPointerException e){
                orderValuesCaseSensitiveFromTables = false;
            }
            result.add(orderValuesCaseSensitiveFromTables);
        }
        return result;
    }
